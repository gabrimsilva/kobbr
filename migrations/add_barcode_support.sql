-- ============================================
-- MIGRATION: Adicionar suporte a código de barras
-- Data: 2025-01-XX
-- Descrição: Adiciona campos de código de barras para produtos e variantes
-- ============================================

-- 1. Adicionar campo barcode na tabela produtos
ALTER TABLE produtos 
ADD COLUMN IF NOT EXISTS barcode VARCHAR(100) NULL;

-- 2. Criar índice para busca rápida por código de barras em produtos
CREATE INDEX IF NOT EXISTS idx_produtos_barcode ON produtos(barcode) WHERE barcode IS NOT NULL;

-- 3. Adicionar constraint de unicidade para código de barras em produtos
-- (permite NULL, mas se preenchido deve ser único)
ALTER TABLE produtos 
ADD CONSTRAINT unique_produtos_barcode UNIQUE (barcode);

-- 4. Adicionar campo barcode na tabela stock_variants
ALTER TABLE stock_variants 
ADD COLUMN IF NOT EXISTS barcode VARCHAR(100) NULL;

-- 5. Criar índice para busca rápida por código de barras em variantes
CREATE INDEX IF NOT EXISTS idx_stock_variants_barcode ON stock_variants(barcode) WHERE barcode IS NOT NULL;

-- 6. Adicionar constraint de unicidade para código de barras em variantes
-- (permite NULL, mas se preenchido deve ser único)
ALTER TABLE stock_variants 
...
BEGIN
  ...
  RAISE NOTICE '✓ Migration de código de barras concluída com sucesso!';
END $$;
