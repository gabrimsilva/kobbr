...
### 1. Configurações
**Arquivo**: `configuracoes.sql` (a criar)
...
CREATE TABLE configuracoes (
    ...
    categoria TEXT, -- 'geral', 'entrega', 'pagamento', 'visual'
    UNIQUE(loja_id, chave)
);
```

### 2. Histórico de Pedidos
**Arquivo**: `historico_pedidos.sql` (a criar)

**Conteúdo**:
- Auditoria de mudanças em pedidos
...
**Arquivo**: `historico_comandas.sql` (a criar)

**Conteúdo**:
- Auditoria de mudanças em comandas
...
CREATE TABLE historico_comandas (
    ...
    tipo_evento TEXT, -- 'item_adicionado', 'item_cancelado', 'status_alterado'
    dados_evento JSONB,
...
### 4. Histórico de PDV
**Arquivo**: `historico_pdv.sql` (a criar)

**Conteúdo**:
- Auditoria de operações do PDV
...
CREATE TABLE historico_pdv (
    id UUID PRIMARY KEY,
    caixa_id UUID REFERENCES pdv_caixas(id),
    tipo_evento TEXT, -- 'abertura', 'fechamento', 'sangria', 'suprimento'
    dados_evento JSONB,
    realizado_por UUID REFERENCES auth.users(id),
    realizado_em TIMESTAMP WITH TIME ZONE
);
...
**Status**: Fase 2 concluída ✅ | Fase 3 pendente ⏳
